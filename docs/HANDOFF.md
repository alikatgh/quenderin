...
| **Mock-UI QA harness** | `scripts/ios_sim_run.sh --mock` / Android twin `scripts/android_emu_run.sh --mock` (debug-only intent extra `quenderin.mock_ui`) (DEBUG env `QUENDERIN_MOCK_UI=1`) walks onboarding → chat → history → settings → agent on a simulator with no model. Chat, Settings, Agent screens visually verified 2026-09-05. |
