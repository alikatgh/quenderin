...
- **#1 per-launch token auth (WS + HTTP)** → 🟡 **IMPLEMENTED, awaiting live verify.** `startDashboardServer` mints a 256-bit token handed ONLY to the trusted renderer (Electron: preload `additionalArguments` → `window.quenderinAuth`; CLI browser: the opened URL's `?token=`). **WS upgrade** requires it (`isAuthorized(extractWsToken(url), token)`, constant-time) on top of the Origin check. **All state-changing HTTP routes** (`POST/PUT/PATCH/DELETE` under `/api/`) now require it too — `X-Auth-Token` header or `?token=` — via middleware in `createApp`; read-only GETs stay open; empty token ⇒ fail closed (this also closes the sibling "state-changing routes unauthenticated" MEDIUM). The renderer sends it from a central `ui/src/lib/api.ts` (`apiFetch` + the WS hook). Pure logic unit-tested (`tests/auth-token.test.ts`) + an integration test (un-tokened `POST /api/agent/intervene` → **401**; `?token=` → 200). **You verify** the real renderer still drives the agent + does model-management (launch the app / `npm run dashboard`).
- **#3 stale `dist/electron/main.js`** → ✅ **FIXED (root cause).** `dist/` is git-ignored and `main: dist/electron/main.js` is built **fresh** from the hardened `electron/main.ts` by `tsc`; the auditor's "stale" file was a leftover build artifact. Added a `prebuild:tsc` (`rimraf dist`) so every build is clean — a stale entry can never ship. Verified a fresh `dist/electron/main.js` carries the sandbox + nav guards + token wiring. (The dead/divergent `src/electron/main.ts` is unreferenced — flagged for deletion separately, per the delete-audit protocol.)
- **#9 `asar:false` + unsigned** → 🟡 **asar FIXED; signing needs your certs.** `electron-builder.yaml` now sets `asar: true` with `asarUnpack` for the native addons (`**/*.node` + node-llama-cpp/sharp/hnswlib/tesseract) so the app ships as a read-only archive instead of loose editable files. **You verify by packaging** (`npm run electron:build`). Code signing + notarization require **your developer certificates** (documented inline in the yaml) — can't be committed.
...
- **Agent loop has no wall-clock timeout** (LOW) → ✅ **FIXED.** `runAgentLoop` now takes a `maxWallClockMs` (default 8 min, `QUENDERIN_MAX_AGENT_MS` override) checked before each step, so a mission whose individual steps are slow can't pin the device indefinitely under the step-count cap alone. Unit-tested (`maxWallClockMs=0` → stops before step 1 with a timeout).
...
## Resolution (2026-06-30) — last open HIGH fixed + build break repaired

- **#6 Apple downloader byte-by-byte** → ✅ **FIXED.** The foreground `URLSessionModelDownloader` iterated `URLSession.bytes` one `UInt8` at a time (one async suspension per byte → billions for a multi-GB GGUF, pegging a CPU core and capping throughput). Replaced with a `URLSessionDataDelegate` that writes the OS's native ~16–64 KB `Data` chunks, preserving fractional progress and the C3 integrity gate. Added a `URLProtocol` integration test driving the **real** downloader end-to-end (file written, monotonic progress → 1.0, integrity-gated, `.partial` cleaned up, transport-failure path). `swift build` green, **199 tests pass**. (commit `7111df6`)
- **Build break — not an audit finding (the agents don't compile the code)** → ✅ **FIXED.** `swift build` was failing on current `main` (exit 1): a11y commit `3e3805e` left a bare `@MainActor` method reference (`offsets…forEach(coordinator.delete)`) passed to the rethrows `forEach`, which Swift 6 rejects with a spurious "call can throw." Replaced with an explicit loop that snapshots stable ids before deleting (also avoids an index-shift bug). The iOS/macOS app compiles again. (commit `0bdcd59`)

**Net (2026-06-30): all 10 HIGHs are resolved or mitigated in code.** The only residual is **#1's live-renderer verification** — the per-launch token auth is implemented and unit/integration-tested, but confirming the real Electron renderer still drives the agent needs the running app (can't be done headlessly here). The MEDIUM/LOW/INFO tiers were re-checked opportunistically while fixing the above; a full sweep of those is the remaining audit follow-up.
