...
| **iOS** (Swift) | 🚀 **Live on the App Store** (0.2.0, free — [id6789854363](https://apps.apple.com/app/id6789854363), 2026-07-20). M1–M4 brain + picker + SwiftUI; **app builds + runs on the simulator**; mock by default, **real `LlamaEngine` when llama.cpp is linked** | `LlamaEngine` (real llama.cpp C-API — **links + runs via xcframework**; `DefaultInferenceEngine.make()` picks it when `canImport(llama)`, else mock) | `cd apple/QuenderinKit && swift test` → **289 tests** (incl. model-gated real-inference + multi-turn KV-reuse equivalence tests that run through the actual engine when a model is linked) |
| **Android** (Kotlin) | M1–M4 brain + picker; mock by default, **real `LlamaEngine` when `jni/llama.cpp` is present** | `LlamaEngine` (JNI to llama.cpp — **builds + runs**; `build.gradle.kts` auto-detects `jni/llama.cpp` → ships `libquenderin_llama.so`, else mock) | `android/quenderin-core` via bundled `kotlinc` → **253 checks**; `./gradlew :app:assembleDebug` → APK |
...
- **Conversation export** = `ConversationExporter` renders a transcript to portable Markdown, shared on the user's terms (iOS `ShareLink` in `ChatView`; Android `ACTION_SEND` from `ChatScreen`). Both platforms. Its agent twin **`AgentRunExporter`** renders an agent run as a reviewable Markdown *walkthrough* (steps · tools · observations · halt reason) — a shareable on-device artifact (the one idea worth borrowing from agentic IDEs like Antigravity), pure + tested.
